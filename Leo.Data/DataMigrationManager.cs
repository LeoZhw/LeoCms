...
using YesSql;
using Leo.Abstractions.Features;
...
namespace Leo.Data
{
    public class DataMigrationManager : IDataMigrationManager
    {
        ...
        public async Task<IEnumerable<string>> GetFeaturesThatNeedUpdateAsync()
        {
            ...
            var outOfSchemaBuilders = _schemaBuilders.Where(schemaBuilder =>
            {
                DataMigration record;
                if (currentVersions.TryGetValue(schemaBuilder.GetType().FullName, out record) && record.Version.HasValue)
                {
                    return CreateUpgradeLookupTable(schemaBuilder).ContainsKey(record.Version.Value);
                }

                return ((GetCreateMethod(schemaBuilder) ?? GetCreateAsyncMethod(schemaBuilder)) != null);
            });

            return outOfSchemaBuilders.Select(m => _featureManager.GetFeatureForDependency(m.GetType()).Id).ToList();
        }
        ...
        public async Task Uninstall(string feature)
        {
            ...
            var schemaBuilders = GetDataMigrations(feature);

            // apply update methods to each migration class for the module
            foreach (var schemaBuilder in schemaBuilders)
            {
                // copy the object for the Linq query
                var tempSchema = schemaBuilder;

                // get current version for this migration
                var dataMigrationRecord = await GetDataMigrationRecordAsync(tempSchema);

                var uninstallMethod = GetUninstallMethod(schemaBuilder);
                if (uninstallMethod != null)
                {
                    uninstallMethod.Invoke(schemaBuilder, new object[0]);
                }

                var uninstallAsyncMethod = GetUninstallAsyncMethod(schemaBuilder);
                if (uninstallAsyncMethod != null)
                {
                    await (Task)uninstallAsyncMethod.Invoke(schemaBuilder, new object[0]);
                }
        ...
        public async Task UpdateAsync(string featureId)
        {
            ...
            var dependencies = _featureManager
            ...
            {
                var schemaBuilder = new YesSql.Sql.SchemaBuilder(_store.Configuration, await _session.DemandAsync());
        ...
        private async Task<DataMigration> GetDataMigrationRecordAsync(ISchemaBuilder tempMigration)
        {
        ...
        /// <summary>
        /// Returns all the available ISchemaBuilder instances for a specific module, and inject necessary builders
        /// </summary>
        private IEnumerable<ISchemaBuilder> GetDataMigrations(string featureId)
        {
            var migrations = _schemaBuilders
                    .Where(dm => _featureManager.GetFeatureForDependency(dm.GetType()).Id == featureId)
                    .ToList();
        ...
        /// </summary>
        private static Dictionary<int, MethodInfo> CreateUpgradeLookupTable(ISchemaBuilder schemaBuilder)
        {
        ...
        /// </summary>
        private static MethodInfo GetCreateMethod(ISchemaBuilder schemaBuilder)
        {
            var methodInfo = schemaBuilder.GetType().GetMethod("Create", BindingFlags.Public | BindingFlags.Instance);
        ...
        /// </summary>
        private static MethodInfo GetCreateAsyncMethod(ISchemaBuilder schemaBuilder)
        {
            var methodInfo = schemaBuilder.GetType().GetMethod("CreateAsync", BindingFlags.Public | BindingFlags.Instance);
        ...
        /// </summary>
        private static MethodInfo GetUninstallMethod(ISchemaBuilder schemaBuilder)
        {
            var methodInfo = schemaBuilder.GetType().GetMethod("Uninstall", BindingFlags.Public | BindingFlags.Instance);
        ...
        /// </summary>
        private static MethodInfo GetUninstallAsyncMethod(ISchemaBuilder schemaBuilder)
        {
            var methodInfo = schemaBuilder.GetType().GetMethod("UninstallAsync", BindingFlags.Public | BindingFlags.Instance);
